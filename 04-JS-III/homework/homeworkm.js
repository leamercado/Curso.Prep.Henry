...
function incrementarPorUno(array) {
  ...
  // y devuelve el array
  // Tu código:
  for (let i=0; i<array.length;i++){  // recordar usar ; en FOR
    //return array++;   MAL
    array[i]++;
   // array[i]=array[i] +1 tambien puede ser
  }
  return array; // no es necesario pasarle parametros
}
...
function agregarItemAlFinalDelArray(array, elemento) {
  ...
  // y devuelve el array
  // Tu código:           push anade al final.   unshit anade al principio
  //                      pop elimina al final   shit elimina al principio
  //    no se hace con for (let i=0; i<array.length;i++){}
  array.push(elemento);
  return array;
}
...
function arrayContiene(array, elemento) {
  ...
  // Tu código:
  //Usando la técnica de acceso al índice ("index access technique") 
  //podemos acceder a cada elemento de la matriz. Para hacer esto, 
  //usamos el método .length como punto de parada para el ciclo.
  for (let i=0; i<array.length; i++){
    if (array[i]===elemento){   // USO === para hacer comparacion, no asigno
      return true;
    }
  } 
  return false;
...
function promedioResultadosTest(resultadosTest) {
  ...
  // Tu código:  PROMEDIO = SUMA DE N /N
  var promedio=0;
  var suma =0;
...
function diaDeLaSemana(numeroDeDia) {
  ...
  //Escribe tu código aquí:
  if (numeroDeDia === 7 || numeroDeDia===1){     // OJO con ===
...
function empiezaConNueve(n) {
  ...
  //Escribe tu código aquí    paso n a STRING 
  // n=90
  // n="90"
  // n[0]=9   IMPORTANTE :)
  var resultado= n.toString();
  if (resultado[0]=== "9"){  // n es ahora un string
    return true;
  }
    return false;  
}
  ...
  var iguales= [];
    for (let i=0; i< arreglo.length; i++){
      if (arreglo[i]=== arreglo[0]){
        iguales.push(arreglo[i]);   // ASI se pushea 
      }
...
function mesesDelAño(array) {
  ...
  // Tu código:
  // array =["Enero", "Marzo" , "Septiembre" , "Noviembre"]
  var meses=[];
  for (let i =0 ; i< array.length; i++){
    if (array[i]=== "Enero" || array[i]=== "Marzo" || array[i]=== "Noviembre"){  // BUSCO EN i= 0 i=1...
      meses.push(array[i]);    // PUSHEO :)
    }
    if (meses.length===3){
      return meses;
    }
  }     // PRESTAR ATENCION A LAS LLAVES
  return "No se encontraron los meses pedidos"  // FUERA DEL FOR
}
